...
export default async function handler(req, res) {
  ...
  if (error) {
    return res.redirect(`${process.env.NEXTAUTH_URL}/Import?gmail_error=${error}`);
  }

  if (!code) {
    return res.redirect(`${process.env.NEXTAUTH_URL}/Import?gmail_error=no_code`);
  }
  ...
  try {
    ...
    if (tokens.error) {
      ...
      return res.redirect(`${process.env.NEXTAUTH_URL}/Import?gmail_error=token_exchange`);
    }
    ...
    if (!supabaseRes.ok) {
      ...
      return res.redirect(`${process.env.NEXTAUTH_URL}/Import?gmail_error=store_failed`);
    }

    // Success — redirect back to Import page
    res.redirect(`${process.env.NEXTAUTH_URL}/Import?gmail_connected=${encodeURIComponent(gmailAddress)}`);

  } catch (err) {
    console.error('OAuth callback error:', err);
    res.redirect(`${process.env.NEXTAUTH_URL}/Import?gmail_error=unknown`);
  }
}

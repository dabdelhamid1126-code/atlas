  ...
  const loadData = async () => {
    try {
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        trend.push({ month: format(md, 'MM/dd'), spent, revenue, profit });
      }
      setTrendData(trend);
      ...
      orders.forEach(o => {
        if (o.credit_card_id) {
          if (!cardMap[o.credit_card_id]) {
            ...
            cardMap[o.credit_card_id] = { name: card?.card_name || 'Unknown', spent: 0, orders: 0 };
          }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        <MetricCard
          ...
          sub={`${allOrders.length} total items`}
          color="blue"
      ...
      {/* Recent Transactions */}
      <RecentTransactions orders={[...allOrders].sort((a, b) => new Date(b.order_date || b.created_date) - new Date(a.order_date || a.created_date))} />
    </div>
  );
}
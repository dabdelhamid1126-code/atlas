...
/* -- Exact hex values from Dashboard -- */
const C = {
  ink:        '#1e1a14',
  inkDim:     '#5a4e3a',
  inkFaded:   '#8a7a60',
  ...
  gold:       '#c9a84c',
  gold2:      '#a07828',
  goldBg:     'rgba(160,114,42,0.10)',
  goldBdr:    'rgba(160,114,42,0.22)',
  parchCard:  '#faf5ee',
  parchWarm:  '#f0e8d8',
  parchLine:  '#e4d8c8',
  ocean:      '#2a5c7a',
  oceanBg:    'rgba(42,92,122,0.10)',
  oceanBdr:   'rgba(42,92,122,0.22)',
  terrain:    '#4a7a35',
  terrainBg:  'rgba(74,122,53,0.10)',
  terrainBdr: 'rgba(74,122,53,0.20)',
  violet:     '#5a3a6e',
  violetBg:   'rgba(90,58,110,0.10)',
  violetBdr:  'rgba(90,58,110,0.20)',
  crimson:    '#8b3a2a',
  crimsonBg:  'rgba(139,58,42,0.10)',
  crimsonBdr: 'rgba(139,58,42,0.20)',
};
...
const CAT = {
  phones:      { bg:C.oceanBg,   color:C.ocean,   border:C.oceanBdr   },
  tablets:     { bg:C.terrainBg, color:C.terrain,  border:C.terrainBdr },
  laptops:     { bg:C.violetBg,  color:C.violet,   border:C.violetBdr  },
  gaming:      { bg:C.crimsonBg, color:C.crimson,  border:C.crimsonBdr },
  accessories: { bg:C.goldBg,    color:C.gold2,    border:C.goldBdr    },
  wearables:   { bg:'rgba(26,64,96,0.10)',   color:'#1a4060', border:'rgba(26,64,96,0.22)'   },
  audio:       { bg:C.violetBg,  color:C.violet,   border:C.violetBdr  },
  other:       { bg:'rgba(138,122,96,0.10)', color:C.inkFaded, border:'rgba(138,122,96,0.22)' },
};

const SRC = {
  'UPCitemdb': { bg:C.oceanBg,   color:C.ocean,   border:C.oceanBdr   },
  'Best Buy':  { bg:C.goldBg,    color:C.gold2,   border:C.goldBdr    },
  'Google':    { bg:C.terrainBg, color:C.terrain,  border:C.terrainBdr },
};

const INP = { background:C.parchWarm, border:`1px solid ${C.parchLine}`, borderRadius:8, color:C.ink, padding:'8px 12px', fontSize:13, outline:'none', width:'100%' };
...
function SectionDivider({ title, dotColor=C.gold, lineColor='rgba(160,114,42,0.25)' }) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:7, marginBottom:12, marginTop:4 }}>
      ...
      <span style={{ fontFamily:SERIF, fontSize:9, fontWeight:700, letterSpacing:'0.12em', textTransform:'uppercase', color:dotColor, whiteSpace:'nowrap' }}>{title}</span>
      <div style={{ flex:1, height:1, background:`linear-gradient(90deg,${lineColor},rgba(160,114,42,0.06),transparent)` }}/>
    </div>
  );
}
...
function LBL({ children }) {
  return (
    <label style={{ fontFamily:SERIF, fontSize:9, fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', color:C.inkFaded, display:'block', marginBottom:4 }}>
      {children}
    </label>
  );
}
...
function CategoryBadge({ category }) {
  ...
  return (
    <span style={{ display:'inline-flex', alignItems:'center', padding:'3px 9px', borderRadius:99, fontSize:9, fontWeight:800, letterSpacing:'0.07em', textTransform:'uppercase', background:s.bg, color:s.color, border:`1px solid ${s.border}`, fontFamily:SANS }}>
      {category || 'other'}
    </span>
  );
}
...
function SourceBadge({ source }) {
  ...
  return (
    <span style={{ display:'inline-flex', alignItems:'center', padding:'3px 9px', borderRadius:99, fontSize:9, fontWeight:800, letterSpacing:'0.06em', textTransform:'uppercase', background:s.bg, color:s.color, border:`1px solid ${s.border}`, whiteSpace:'nowrap', flexShrink:0, fontFamily:SANS }}>
      {source}
    </span>
  );
}
...
function ProductImage({ src, name, size=60 }) {
  ...
  return (
    <div style={{ width:size, height:size, borderRadius:10, background:C.terrainBg, border:`1px solid ${C.terrainBdr}`, display:'flex', alignItems:'center', justifyContent:'center' }}>
      <span style={{ fontSize:size*0.4, fontWeight:700, color:C.terrain, fontFamily:SERIF }}>{name?.charAt(0)?.toUpperCase()||'?'}</span>
    </div>
  );
}
...
function ProductThumb({ src, name, size=38 }) {
  ...
  return (
    <div style={{ width:size, height:size, borderRadius:8, flexShrink:0, background:C.terrainBg, border:`1px solid ${C.terrainBdr}`, display:'flex', alignItems:'center', justifyContent:'center' }}>
      <span style={{ fontSize:size*0.38, fontWeight:700, color:C.terrain, fontFamily:SERIF }}>{name?.charAt(0)?.toUpperCase()||'?'}</span>
    </div>
  );
}
...
function ProductCard({ product, onEdit, onDelete }) {
  ...
  return (
    <div
      ...
      style={{ background:hovered?C.goldBg:C.parchCard, border:`1px solid ${hovered?C.goldBdr:C.parchLine}`, borderRadius:12, padding:'12px 10px', display:'flex', flexDirection:'column', alignItems:'center', gap:8, textAlign:'center', position:'relative', transition:'all 0.15s', cursor:'default' }}
    >
      {hovered && (
      ...
      <div title={product.name} style={{ fontSize:11, fontWeight:600, color:C.ink, lineHeight:1.35, maxWidth:'100%', overflow:'hidden', display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', fontFamily:SANS }}>
        {display}
      </div>
...
function UPCPickerModal({ upc, results, onSelect, onManual, onClose }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:1100, display:'flex', alignItems:'center', justifyContent:'center' }}>
      ...
      <div style={{ position:'relative', width:'100%', maxWidth:520, background:C.parchCard, borderRadius:16, border:`1px solid ${C.parchLine}`, boxShadow:'0 24px 60px rgba(0,0,0,0.2)', overflow:'hidden' }}>
        <div style={{ padding:'16px 20px', borderBottom:`1px solid ${C.parchLine}`, display:'flex', alignItems:'flex-start', justifyContent:'space-between', background:C.parchWarm }}>
          <div>
            <div style={{ fontFamily:SERIF, fontSize:14, fontWeight:700, color:C.ink }}>
              {results.length>1?'Multiple results found':'Result found'}
            </div>
            <div style={{ fontSize:10, color:C.inkDim, marginTop:2, fontFamily:SANS }}>
        ...
        <div style={{ padding:'14px 20px', display:'flex', flexDirection:'column', gap:7, maxHeight:'60vh', overflowY:'auto' }}>
          {results.map((r,i)=>(
            <div key={i} onClick={()=>setSelected(i)}
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontSize:12, fontWeight:600, color:C.ink, lineHeight:1.35, fontFamily:SANS }}>{cleanDisplayName(r.title)}</div>
          ...
          <div onClick={onManual} style={{ borderRadius:10, padding:'9px 12px', border:`1px solid ${C.crimsonBdr}`, background:C.crimsonBg, fontSize:11, color:C.crimson, cursor:'pointer', textAlign:'center', fontFamily:SANS }}>
        ...
        <div style={{ padding:'12px 20px', borderTop:`1px solid ${C.parchLine}`, background:C.parchWarm, display:'flex', justifyContent:'flex-end', gap:8 }}>
          <button onClick={onClose} style={{ padding:'8px 16px', borderRadius:99, fontSize:12, fontWeight:500, background:C.parchWarm, border:`1px solid ${C.parchLine}`, color:C.inkFaded, cursor:'pointer', fontFamily:SANS }}>Cancel</button>
          <button onClick={()=>onSelect(results[selected])} style={{ padding:'8px 18px', borderRadius:99, fontSize:12, fontWeight:700, background:C.ink, color:C.gold, border:'none', cursor:'pointer', fontFamily:SERIF }}>
            Use selected
          </button>
        </div>
      </div>
...
function Pagination({ page, totalPages, total, onPage }) {
  ...
  const btnS = (active) => ({ width:32, height:32, borderRadius:99, display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:active?700:400, cursor:'pointer', border:'1px solid', background:active?C.goldBg:C.parchCard, borderColor:active?C.goldBdr:C.parchLine, color:active?C.gold2:C.inkDim, fontFamily:MONO });
  const navS = (disabled) => ({ width:32, height:32, borderRadius:99, display:'flex', alignItems:'center', justifyContent:'center', cursor:disabled?'not-allowed':'pointer', border:`1px solid ${C.parchLine}`, background:C.parchCard, color:disabled?C.inkGhost:C.inkFaded, opacity:disabled?0.4:1 });
  return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginTop:20, flexWrap:'wrap', gap:10 }}>
      <span style={{ fontSize:11, color:C.inkDim, fontFamily:SANS }}>Showing {start}-{end} of {total} products</span>
...
export default function Products() {
  ...
  const th  = { color:C.inkDim, fontSize:9, textTransform:'uppercase', letterSpacing:'0.06em', padding:'8px 10px', textAlign:'left', borderBottom:`1px solid ${C.parchLine}`, fontWeight:700, fontFamily:SERIF };
  const tdS = { padding:'8px', borderBottom:`1px solid ${C.parchLine}`, verticalAlign:'middle' };
  ...
  return (
    <div style={{ paddingBottom:24 }}>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:20 }}>
        <div>
          <h1 style={{ fontFamily:SERIF, fontSize:24, fontWeight:900, color:C.ink, letterSpacing:'-0.3px', margin:0 }}>Products</h1>
          <p style={{ fontSize:11, color:C.inkDim, marginTop:4, fontFamily:SANS }}>Master product catalog  {products.length} total  shared with all users</p>
        </div>
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          <button onClick={()=>setImportOpen(true)}
            style={{ display:'inline-flex', alignItems:'center', gap:6, padding:'9px 16px', borderRadius:99, fontSize:12, fontWeight:600, background:C.parchWarm, border:`1px solid ${C.parchLine}`, color:C.inkDim, cursor:'pointer', fontFamily:SANS }}>
            <Upload style={{ width:13, height:13 }}/> Import UPCs
          </button>
          <button onClick={()=>openDialog()}
            style={{ display:'inline-flex', alignItems:'center', gap:6, padding:'9px 18px', borderRadius:99, fontSize:12, fontWeight:700, background:C.ink, color:C.gold, border:'none', cursor:'pointer', fontFamily:SERIF, letterSpacing:'0.04em' }}>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:16, alignItems:'center', background:C.parchCard, border:`1px solid ${C.parchLine}`, borderRadius:12, padding:'8px 12px', flexWrap:'wrap' }}>
          ...
          {['all',...CATEGORIES].map(cat=>{
            ...
            return (
              <button key={cat} onClick={()=>{ setCategoryFilter(cat); setPage(1); }}
                style={{ padding:'5px 12px', borderRadius:99, fontSize:10, fontWeight:700, letterSpacing:'0.05em', textTransform:'uppercase', cursor:'pointer', border:'1px solid', fontFamily:SANS,
                  background:  active?(s?.bg||C.terrainBg):'transparent',
                  color:       active?(s?.color||C.terrain):C.inkGhost,
                  borderColor: active?(s?.border||C.terrainBdr):C.parchLine,
                }}>
                {cat}
              </button>
            );
      ...
      {isLoading ? (
        ...
        <div style={{ textAlign:'center', padding:'48px 24px', background:C.parchCard, borderRadius:12, border:`1px solid ${C.parchLine}` }}>
          <Package style={{ width:36, height:36, color:C.inkGhost, margin:'0 auto 12px' }}/>
          <p style={{ color:C.inkDim, fontSize:13, fontFamily:SANS }}>No products found.</p>
        </div>
      ) : (
        <>
      ...
      {dialogOpen && (
        <div style={{ position:'fixed', inset:0, zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center' }}>
          ...
          <div style={{ position:'relative', width:'100%', maxWidth:480, background:C.parchCard, borderRadius:16, border:`1px solid ${C.parchLine}`, boxShadow:'0 24px 60px rgba(0,0,0,0.15)', overflow:'hidden' }}>
            <div style={{ padding:'18px 24px 14px', borderBottom:`1px solid ${C.parchLine}`, background:C.parchWarm, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              <div>
                <h2 style={{ fontFamily:SERIF, fontSize:16, fontWeight:700, margin:0, color:C.ink }}>{editingProduct?'Edit Product':'Add Product'}</h2>
                <p style={{ fontSize:10, color:C.inkGhost, marginTop:2, fontFamily:SANS }}>{editingProduct?'Update product details':'Visible to all users once created'}</p>
              </div>
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ padding:'18px 24px', display:'flex', flexDirection:'column', gap:14 }}>
                ...
                <div>
                  ...
                  {dupWarning?.field==='name' && <p style={{ fontSize:11, color:C.crimson, marginTop:4, fontFamily:SANS }}>Already in catalog: "{cleanDisplayName(dupWarning.product.name)}"</p>}
                  {formData.name && !dupWarning && <p style={{ fontSize:10, color:C.inkGhost, marginTop:3, fontFamily:SANS }}>Displays as: <span style={{ color:C.inkDim, fontWeight:600 }}>{cleanDisplayName(formData.name)}</span></p>}
                </div>
                <div>
                  <LBL>UPC</LBL>
                  <div style={{ display:'flex', gap:8 }}>
                    ...
                    <button type="button" disabled={loadingUPC} onClick={lookupSingleUPC}
                      style={{ padding:'8px 14px', borderRadius:99, fontSize:12, fontWeight:700, background:C.ink, color:C.gold, border:'none', cursor:'pointer', whiteSpace:'nowrap', opacity:loadingUPC?0.5:1, display:'flex', alignItems:'center', gap:5, fontFamily:SERIF }}>
                      {loadingUPC ? <><div style={{ width:12, height:12, borderRadius:'50%', border:`2px solid ${C.gold}`, borderTopColor:'transparent', animation:'spin 0.8s linear infinite' }}/> Searching...</> : <><Barcode style={{ width:13, height:13 }}/> Lookup</>}
                    </button>
                  </div>
                  {dupWarning?.field==='upc' && <p style={{ fontSize:11, color:C.crimson, marginTop:4, fontFamily:SANS }}>UPC already exists: "{cleanDisplayName(dupWarning.product.name)}"</p>}
                </div>
                <div>
                  <LBL>Category <span style={{ color:C.terrain, fontSize:9, fontWeight:400, textTransform:'none', letterSpacing:0, fontFamily:SANS }}>auto-detected from name</span></LBL>
                  <select value={formData.category} onChange={e=>setFormData({ ...formData, category:e.target.value })} style={{ ...INP, cursor:'pointer', fontFamily:SANS }}>
              ...
              <div style={{ padding:'14px 24px', borderTop:`1px solid ${C.parchLine}`, background:C.parchWarm, display:'flex', justifyContent:'flex-end', gap:10 }}>
                <button type="button" onClick={closeDialog} style={{ padding:'8px 16px', borderRadius:99, fontSize:12, fontWeight:500, background:C.parchWarm, border:`1px solid ${C.parchLine}`, color:C.inkFaded, cursor:'pointer', fontFamily:SANS }}>Cancel</button>
                <button type="submit" disabled={!!dupWarning} style={{ padding:'8px 20px', borderRadius:99, fontSize:12, fontWeight:700, background:C.ink, color:C.gold, border:'none', cursor:dupWarning?'not-allowed':'pointer', opacity:dupWarning?0.5:1, fontFamily:SERIF }}>
      ...
      {importOpen && (
        <div style={{ position:'fixed', inset:0, zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center' }}>
          ...
          <div style={{ position:'relative', width:'100%', maxWidth:780, maxHeight:'88vh', background:C.parchCard, borderRadius:16, border:`1px solid ${C.parchLine}`, boxShadow:'0 24px 60px rgba(0,0,0,0.15)', display:'flex', flexDirection:'column', overflow:'hidden' }}>
            <div style={{ padding:'18px 24px 14px', borderBottom:`1px solid ${C.parchLine}`, background:C.parchWarm, display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
              <div>
                <h2 style={{ fontFamily:SERIF, fontSize:16, fontWeight:700, margin:0, color:C.ink }}>Import UPCs</h2>
                <p style={{ fontSize:11, color:C.inkDim, marginTop:2, fontFamily:SANS }}>Best Buy preferred  category auto-detected  duplicates skipped</p>
              </div>
            ...
            <div style={{ flex:1, overflowY:'auto', padding:'18px 24px' }}>
              {importRows.length===0 ? (
                <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                  ...
                  <p style={{ fontSize:11, color:C.inkDim, fontFamily:SANS }}>Tries Best Buy then UPCitemdb then Google  category auto-detected  you can override before importing</p>
                </div>
              ) : (
                <div>
                  <div style={{ display:'flex', gap:8, marginBottom:14, flexWrap:'wrap' }}>
                    {newCount >0&&<span style={{ padding:'3px 10px', borderRadius:99, fontSize:11, fontWeight:700, background:C.terrainBg, color:C.terrain, border:`1px solid ${C.terrainBdr}`, fontFamily:SANS }}>{newCount} new</span>}
                    {dupCount >0&&<span style={{ padding:'3px 10px', borderRadius:99, fontSize:11, fontWeight:700, background:C.goldBg,    color:C.gold2,   border:`1px solid ${C.goldBdr}`,    fontFamily:SANS }}>{dupCount} already exist</span>}
                    {failCount>0&&<span style={{ padding:'3px 10px', borderRadius:99, fontSize:11, fontWeight:700, background:C.crimsonBg, color:C.crimson, border:`1px solid ${C.crimsonBdr}`, fontFamily:SANS }}>{failCount} not found</span>}
                    {lookingUp&&<span  style={{ padding:'3px 10px', borderRadius:99, fontSize:11, fontWeight:700, background:C.oceanBg,   color:C.ocean,   border:`1px solid ${C.oceanBdr}`,   fontFamily:SANS }}>Looking up...</span>}
                  </div>
                      ...
                      {importRows.map(row=>{
                        ...
                        return (
                          <tr key={row.upc} style={{ opacity:(isDup||isFail)?0.4:1 }}>
                            <td style={{ ...tdS, textAlign:'center' }}>{isNew&&<input type="checkbox" checked={row.selected} onChange={()=>toggleRow(row.upc)}/>}</td>
                            <td style={tdS}>{isLoad?<div style={{ width:32, height:32, borderRadius:6, background:C.parchWarm, display:'flex', alignItems:'center', justifyContent:'center' }}><Loader style={{ width:14, height:14, color:C.ocean, animation:'spin 1s linear infinite' }}/></div>:<ProductThumb src={row.image} name={row.name} size={32}/>}</td>
                            <td style={tdS}>{isNew?<div><input value={cleanDisplayName(row.name)} onChange={e=>updateImportRow(row.upc,'name',e.target.value)} style={{ ...INP, padding:'4px 8px', fontSize:11 }}/>{row.name&&cleanDisplayName(row.name)!==row.name&&<p style={{ fontSize:9, color:C.inkGhost, marginTop:2, fontFamily:MONO }} title={row.name}>Full: {row.name.slice(0,40)}...</p>}</div>:<span style={{ fontSize:11, color:C.inkDim, fontFamily:SANS }}>{cleanDisplayName(row.name)||'--'}</span>}</td>
                            <td style={tdS}><span style={{ fontFamily:MONO, fontSize:10, color:C.inkGhost }}>{row.upc}</span></td>
                            <td style={tdS}>{isNew&&<select value={row.category} onChange={e=>updateImportRow(row.upc,'category',e.target.value)} style={{ ...INP, padding:'4px 6px', fontSize:11, cursor:'pointer', fontFamily:SANS }}><option value="">--</option>{CATEGORIES.map(c=><option key={c} value={c}>{c}</option>)}</select>}</td>
                            <td style={tdS}>{row.source&&<SourceBadge source={row.source}/>}</td>
                            <td style={tdS}>
                              {isLoad&&<span style={{ fontSize:10, color:C.ocean,   fontFamily:SANS }}>Looking...</span>}
                              {isNew &&<span style={{ fontSize:10, fontWeight:700, color:C.terrain, fontFamily:SANS }}>New</span>}
                              {isDup &&<span style={{ fontSize:10, fontWeight:700, color:C.gold2,   fontFamily:SANS }}>Exists</span>}
                              {isFail&&<span style={{ fontSize:10, fontWeight:700, color:C.crimson, fontFamily:SANS }}>Not found</span>}
                            </td>
                          </tr>
                        );
                      })}
            ...
            <div style={{ padding:'14px 24px', borderTop:`1px solid ${C.parchLine}`, background:C.parchWarm, display:'flex', justifyContent:'space-between', alignItems:'center', flexShrink:0 }}>
              <div style={{ fontSize:11, color:C.inkDim, fontFamily:SANS }}>{importRows.length>0&&!lookingUp&&`${selCount} product${selCount!==1?'s':''} selected`}</div>
              <div style={{ display:'flex', gap:10 }}>
                {importRows.length>0&&!lookingUp&&<button onClick={()=>{ setImportRows([]); setUpcInput(''); }} style={{ padding:'8px 16px', borderRadius:99, fontSize:12, fontWeight:500, background:C.parchWarm, border:`1px solid ${C.parchLine}`, color:C.inkFaded, cursor:'pointer', fontFamily:SANS }}>Back</button>}
                {importRows.length===0&&<button onClick={handleBulkLookup} disabled={!upcInput.trim()||lookingUp} style={{ padding:'8px 20px', borderRadius:99, fontSize:12, fontWeight:700, background:upcInput.trim()?C.ink:C.parchWarm, border:upcInput.trim()?'none':`1px solid ${C.parchLine}`, color:upcInput.trim()?C.gold:C.inkGhost, cursor:upcInput.trim()?'pointer':'not-allowed', fontFamily:SERIF }}>Look up UPCs</button>}
                {importRows.length>0&&!lookingUp&&selCount>0&&<button onClick={handleBulkImport} disabled={importing} style={{ padding:'8px 20px', borderRadius:99, fontSize:12, fontWeight:700, background:C.ink, color:C.gold, border:'none', cursor:'pointer', opacity:importing?0.7:1, fontFamily:SERIF }}>{importing?'Importing...':`Import ${selCount} product${selCount!==1?'s':''}`}</button>}
              </div>
            </div>
          </div>
        </div>

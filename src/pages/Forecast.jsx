...
import {
  Calculator, TrendingUp, BarChart2,
  Store, ChevronUp, ChevronDown, Target, RefreshCw,
  Search, X, Package, Barcode,
} from 'lucide-react';
import {
      ...
      if (isUpc) {
        ...
        try {
          ...
          if (liveOffers.length > 0) {
            setResults(prev => prev.map(r => ({ ...r, offers: liveOffers, loadingLivePrices: false })));
          } else {
  ...
  return (
    <div style={{ marginBottom: 16, position: 'relative' }}>
      ...
      <div style={{ position: 'relative', display: 'flex', gap: 6 }}>
        ...
        >
          {loading ? <RefreshCw size={12} className="spin" /> : isUpc ? <Barcode size={13} /> : <Search size={13} />}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              {selectedProduct.offers.map((offer, idx) => (
                ...
                >
                  ...
                  <button
                    onClick={e => { e.stopPropagation(); setUnitCost(String(offer.price)); }}
                    style={{
      ...
      {hasInput && mode === 'breakeven' && (
        <div style={{ borderRadius: 12, border: '1px solid var(--gold-bdr)', padding: 14, background: 'var(--gold-bg)' }}>
          ...
          <p style={{ fontSize: 11, color: 'var(--ink-dim)', lineHeight: 1.5 }}>
            ...
            {cashbackEarned > 0 ? ` and ${pct(cashbackRate)} cashback (${fmt$(cashbackEarned)})` : ''}, you need at least{' '}
            <strong style={{ color: 'var(--gold)', fontFamily: 'var(--font-mono)' }}>{fmt$(breakEvenPer)}</strong> per unit.
          </p>
        </div>
      )}
    </SectionCard>
...
function MonthlyProjection({ orders, rewards, goals }) {
  ...
  const daysLeft = Math.ceil((endOfMonth(now) - now) / (1000 * 60 * 60 * 24));
    ...
    <SectionCard icon={Target} title="Goal Calculator - What Do I Need?" accentColor="var(--gold)">
      ...
      {hasInput && !feasible && (
        <div style={{ borderRadius: 10, padding: '12px 14px', background: 'var(--crimson-bg)', border: '1px solid var(--crimson-bdr)', fontSize: 12, color: 'var(--crimson)' }}>
          At these numbers each order loses money ({fmt$(profitPerOrder)}). Raise the sale price, lower the cost, or reduce fees.
        </div>
      )}
      {hasInput && feasible && (
        <div style={{ borderRadius: 12, border: '1px solid var(--gold-bdr)', padding: 14, background: 'var(--gold-bg)' }}>

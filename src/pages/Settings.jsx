...
// ── Goals Tab ──────────────────────────────────────────────────────────────────
const GOAL_METRICS = [
  { metric: 'profit',       label: 'Profit',       color: 'var(--terrain)' },
  { metric: 'revenue',      label: 'Revenue',      color: 'var(--ocean)'   },
  { metric: 'cashback',     label: 'Cashback',     color: 'var(--violet)'  },
  { metric: 'transactions', label: 'Transactions', color: 'var(--gold)'    },
];
              ...
              <div style={{ display: 'flex', gap: 2, padding: 3, borderRadius: 8, background: 'var(--parch-warm)', border: '1px solid var(--parch-line)' }}>
                {['WEEKLY', 'MONTHLY'].map(p => (
                  <button key={p} onClick={() => update(idx, { period: p })}
                    style={{ padding: '4px 10px', borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: 'pointer', border: 'none', background: goal.period === p ? 'var(--ink)' : 'transparent', color: goal.period === p ? 'var(--gold)' : 'var(--ink-dim)' }}>
    ...
    <div>
      ...
      <div style={card}>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 12, fontSize: 11 }}>
          <button onClick={() => setSel(Object.fromEntries(EXPORT_ITEMS.map(i => [i.key, true])))}
            style={{ background: 'none', border: 'none', color: 'var(--terrain)', cursor: 'pointer', fontWeight: 600 }}>Select all</button>
          <span style={{ color: 'var(--parch-line)' }}>|</span>
          <button onClick={() => setSel(Object.fromEntries(EXPORT_ITEMS.map(i => [i.key, false])))}
            style={{ background: 'none', border: 'none', color: 'var(--ink-dim)', cursor: 'pointer' }}>Deselect all</button>
        </div>
          ...
          {EXPORT_ITEMS.map(item => (
            <button key={item.key} onClick={() => setSel(s => ({ ...s, [item.key]: !s[item.key] }))}
              ...
              <div>
                ...
                <p style={{ fontSize: 11, color: 'var(--ink-dim)', margin: 0 }}>{item.description}</p>
              </div>
            </button>
          ))}
        </div>
        ...
        <button onClick={handleExport} disabled={!Object.values(sel).some(Boolean) || exporting}
          style={{ ...btnCta, opacity: Object.values(sel).some(Boolean) ? 1 : 0.4 }}>
      ...
      <div style={{ ...card, border: '1px solid var(--crimson-bdr)' }}>
        ...
        {stage === 'idle' && (
          <button onClick={() => setStage('confirm')} style={{ padding: '8px 18px', borderRadius: 8, fontSize: 13, fontWeight: 600, background: 'var(--crimson-bg)', border: '1px solid var(--crimson-bdr)', color: 'var(--crimson)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
            <Trash2 style={{ width: 13, height: 13 }} /> Reset All Data
          </button>
        )}

        {stage === 'confirm' && (
          <div style={{ padding: 14, borderRadius: 10, background: 'var(--crimson-bg)', border: '1px solid var(--crimson-bdr)' }}>
            <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--crimson)', marginBottom: 8 }}>Are you absolutely sure? This is irreversible.</p>
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setStage('typing')} style={{ padding: '7px 16px', borderRadius: 8, fontSize: 13, fontWeight: 700, background: 'var(--crimson2)', border: 'none', color: 'white', cursor: 'pointer' }}>Yes, delete everything</button>
        ...
        {stage === 'typing' && (
          <div style={{ padding: 14, borderRadius: 10, background: 'var(--crimson-bg)', border: '1px solid var(--crimson-bdr)' }}>
            <p style={{ fontSize: 12, color: 'var(--ink-dim)', marginBottom: 8 }}>
              Type <code style={{ background: 'var(--crimson-bg)', color: 'var(--crimson)', padding: '1px 6px', borderRadius: 4, fontFamily: 'monospace', border: '1px solid var(--crimson-bdr)' }}>{CONFIRM}</code> to confirm.
            </p>
            <input type="text" value={resetInput} onChange={e => setRI(e.target.value)} placeholder={CONFIRM} autoFocus style={{ ...inp, marginBottom: 10 }} />
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={handleReset} disabled={resetInput !== CONFIRM} style={{ padding: '7px 16px', borderRadius: 8, fontSize: 13, fontWeight: 700, background: 'var(--crimson2)', border: 'none', color: 'white', cursor: 'pointer', opacity: resetInput !== CONFIRM ? 0.4 : 1 }}>Permanently Delete</button>
              <button onClick={resetAll} style={btnGhost}>Cancel</button>
            </div>
          </div>
        )}
        ...
        {stage === 'deleting' && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--crimson)', fontSize: 13 }}>
            <Loader style={{ width: 15, height: 15, animation: 'spin 0.8s linear infinite' }} /> Deleting all data…
          </div>
        )}

        {stage === 'done' && deleted && (
          <div style={{ padding: 14, borderRadius: 10, background: 'var(--crimson-bg)', border: '1px solid var(--crimson-bdr)' }}>
            <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--crimson)', marginBottom: 8 }}>All data has been deleted.</p>
            {Object.entries(deleted).map(([k, v]) => <p key={k} style={{ fontSize: 11, color: 'var(--ink-dim)', margin: '2px 0' }}>{k}: {v} removed</p>)}
    ...
    <div style={card}>
      ...
      <div style={{ padding: 16, borderRadius: 10, background: 'var(--parch-warm)', border: '1px solid var(--parch-line)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink)' }}>17TRACK</p>
        ...
        {connected && masked && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', borderRadius: 8, background: 'var(--parch-card)', border: '1px solid var(--parch-line)', marginBottom: 12 }}>
            <code style={{ fontSize: 12, color: 'var(--ink-faded)', flex: 1, fontFamily: 'monospace' }}>{masked}</code>
            <button onClick={remove} style={{ fontSize: 11, fontWeight: 600, color: 'var(--crimson)', background: 'var(--crimson-bg)', border: '1px solid var(--crimson-bdr)', borderRadius: 6, padding: '3px 10px', cursor: 'pointer' }}>Remove</button>
          </div>
        )}
        ...
        <div style={{ position: 'relative', marginBottom: 10 }}>
          ...
          <button type="button" onClick={() => setShowKey(!showKey)}
            style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: 'var(--ink-dim)', cursor: 'pointer', padding: 0 }}>
  ...
  return (
    <div style={card}>
      ...
      <SectionRow title="Session" description="Your current browser session">
        <span style={{ fontSize: 11, color: 'var(--ink-dim)' }}>Current</span>
      </SectionRow>
      <SectionRow title="Account" description={user?.email || '—'} noBorder>
        <span style={{ fontSize: 11, color: 'var(--ink-dim)' }}>{user?.role || 'user'}</span>
      </SectionRow>
    </div>
  );
}
...
function AppearanceTab() {
  ...
  const dashToggles = [
    { key: 'splitYACashback',        label: 'Split YA Cashback',   description: 'Show CC and YA cashback as separate KPIs',  icon: Sparkles,   color: 'var(--gold)'   },
    { key: 'costIncludesTaxShipping', label: 'Include Tax in Cost', description: 'Total Cost will include taxes and shipping', icon: DollarSign, color: 'var(--ocean)'  },
    { key: 'showPipeline',            label: 'Status Pipeline',     description: 'Show the status pipeline on the Dashboard',  icon: Inbox,      color: 'var(--violet)' },
    { key: 'showProductImages',       label: 'Show Product Images', description: 'Display product thumbnails in Transactions', icon: Eye,        color: 'var(--ocean2)' },
    { key: 'showGoals',               label: 'Goal Tracker',        description: 'Show the goal tracker on the Dashboard',     icon: Target,     color: 'var(--terrain)'},
  ];

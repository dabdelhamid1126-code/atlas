...
const brandfetch = (domain) => `https://cdn.brandfetch.io/domain/${domain}?c=${BRANDFETCH_CLIENT_ID}`;
...
const unsoldQty = (order, productName) => {
  ...
  const soldQty = (order.sale_events || []).reduce((s, ev) =>
    s + (ev.items || [])
      ...
      .reduce((ss, si) => ss + (parseInt(si.quantity) || 1), 0), 0);
...
const findMatchedProduct = (itemName, itemProductId, products) => {
  ...
  let bestScore = 0.4, bestProduct = null;
  products.forEach(p => {
    if (!p.name) return;
...
function StatusBadge({ status }) {
  ...
  return (
    <span style={{ display:'inline-flex', alignItems:'center', padding:'2px 8px', borderRadius:99, fontSize:9, fontWeight:700, letterSpacing:'0.05em', textTransform:'uppercase', background:s.bg, color:s.color, border:`1px solid ${s.border}`, whiteSpace:'nowrap' }}>
      {label}
    </span>
  );
}
...
function StoreLogo({ retailer, size = 24 }) {
  ...
  return (
    <div style={{ width:size, height:size, borderRadius:'50%', overflow:'hidden', flexShrink:0, background:'var(--parch-warm)', border:'1px solid var(--parch-line)', display:'flex', alignItems:'center', justifyContent:'center' }}>
      {!err
        ? <img src={brandfetch(domain)} alt={retailer} style={{ width:'100%', height:'100%', objectFit:'cover' }} onError={() => setErr(true)} />
        : <span style={{ fontSize:size*0.36, fontWeight:700, color:'var(--ocean)' }}>{initials}</span>}
    </div>
  );
}
...
function ProductImage({ src, name, qty, retailer }) {
  ...
  return (
    <div style={{ position:'relative', flexShrink:0 }}>
      <div style={{ width:38, height:38, borderRadius:8, overflow:'hidden', background:'var(--parch-warm)', border:'1px solid var(--parch-line)', display:'flex', alignItems:'center', justifyContent:'center' }}>
        {src && !imgErr
      ...
      <div style={{ position:'absolute', bottom:-4, right:-4, width:17, height:17, borderRadius:'50%', background:'var(--ocean)', color:'white', fontSize:9, fontWeight:700, display:'flex', alignItems:'center', justifyContent:'center', border:'1.5px solid var(--parch-card)' }}>
        {qty}
      </div>
    </div>
  );
...
function SourceOrderRow({ order, qty, costPerUnit, onEdit }) {
  ...
  const bg = hovered ? 'rgba(138,94,0,0.03)' : 'transparent';
  const cell = (extra = {}) => ({ borderBottom:`1px solid var(--parch-line)`, background:bg, padding:'8px 8px', transition:'background 0.12s', ...extra });
  return (
    <tr onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{ cursor:'pointer' }} onClick={() => onEdit(order)}>
      <td style={cell()} />
      <td style={cell({ paddingLeft:28 })}>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <StoreLogo retailer={order.retailer} size={22} />
          <div style={{ minWidth:0 }}>
            <div style={{ fontSize:10, fontWeight:500, color: hovered ? 'var(--ocean)' : 'var(--ink-faded)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis', transition:'color 0.12s' }}>
              {order.retailer || 'Unknown'} · {order.order_number || order.id?.slice(0,8)}
            </div>
            <div style={{ fontSize:9, color:'var(--ink-ghost)', marginTop:1 }}>
              {fmtDate(order.order_date)} · ×{qty} unit{qty !== 1 ? 's' : ''}
            </div>
          </div>
        </div>
      </td>
      <td style={cell({ fontSize:10, color:'var(--ink-dim)' })}>×{qty}</td>
      <td style={cell({ fontSize:10, color:'var(--ink-dim)' })}>{fmt(costPerUnit)}</td>
      <td style={cell({ fontSize:10, color:'var(--ink-dim)' })}>{fmt(costPerUnit * qty)}</td>
      <td style={cell({ fontSize:10, color:'var(--ink-ghost)' })}>—</td>
      <td style={cell()}><StatusBadge status={order.status} /></td>
      <td style={cell({ textAlign:'right' })}>
        <span style={{ display:'inline-flex', alignItems:'center', gap:3, padding:'3px 8px', borderRadius:6, fontSize:9, fontWeight:500, color:'var(--ocean)', background: hovered ? 'var(--ocean-bg)' : 'transparent', border: hovered ? '1px solid var(--ocean-bdr)' : '1px solid transparent', opacity: hovered ? 1 : 0, transition:'all 0.12s', whiteSpace:'nowrap', pointerEvents:'none' }}>
...
function GroupRow({ group, expanded, onToggle, onEdit }) {
  const estProfit = group.listPrice ? (parseFloat(group.listPrice) - group.avgCostPerUnit) * group.totalQty : null;
  const hd = { background:'var(--parch-warm)', borderBottom:'1px solid var(--parch-line)', padding:'11px 8px' };
  return (
    <>
      <tr onClick={onToggle} style={{ cursor:'pointer' }}>
        <td style={{ ...hd, width:28 }}>
          <div style={{ width:14, height:14, borderRadius:3, background:'var(--parch-line)', display:'flex', alignItems:'center', justifyContent:'center', color:'var(--ink-dim)' }}>
        ...
        <td style={hd}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <div style={{ minWidth:0 }}>
              <div style={{ fontSize:12, fontWeight:600, color:'var(--ink)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{group.productName}</div>
              <div style={{ fontSize:9, color:'var(--ink-dim)', marginTop:2 }}>
                {group.sources.length} source order{group.sources.length !== 1 ? 's' : ''} · {[...new Set(group.sources.map(s => s.order.retailer).filter(Boolean))].join(', ')}
              </div>
            </div>
          </div>
        </td>
        <td style={hd}>
          <div style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', width:22, height:22, borderRadius:'50%', background:'var(--ocean-bg)', color:'var(--ocean)', fontSize:10, fontWeight:700 }}>
            {group.totalQty}
          </div>
        </td>
        <td style={{ ...hd, fontSize:12, color:'var(--ink)', fontWeight:600 }}>{fmt(group.avgCostPerUnit)}</td>
        <td style={{ ...hd, fontSize:12, color:'var(--gold)', fontWeight:700 }}>{fmt(group.totalCost)}</td>
        <td style={hd}>
          {estProfit !== null
            ? <span style={{ display:'inline-flex', padding:'2px 8px', borderRadius:99, fontSize:10, fontWeight:500, background:'var(--terrain-bg)', color:'var(--terrain)' }}>+{fmt(estProfit)}</span>
      ...
      {expanded && group.sources.map(({ order, qty, costPerUnit }, idx) => (
        <SourceOrderRow key={order.id + '-' + idx} order={order} qty={qty} costPerUnit={costPerUnit} onEdit={onEdit} />
      ))}
    </>
  );
}
...
export default function Inventory() {
  ...
  const relevantOrders = useMemo(() =>
    orders.filter(o => ['received','partially_received','ordered','shipped','paid','completed'].includes(o.status)), [orders]);
    ...
    relevantOrders.forEach((order) => {
      (order.items || []).forEach((item) => {
        ...
        if (!map[name]) {
          ...
          const imageUrl = matchedProduct?.image || item.product_image_url || item.product_image || item.image_url || null;
          map[name] = { productName:name, imageUrl, listPrice:item.list_price||item.listing_price||null, sources:[], totalQty:0, totalCost:0, statuses:[] };
        }
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      estProfit:      groups.filter(g => g.listPrice).reduce((s,g) => s + (parseFloat(g.listPrice) - g.avgCostPerUnit) * g.totalQty, 0),
    };
  }, [groups]);
  ...
  return (
    <div style={{ paddingBottom:24 }}>
      ...
      {/* Filters */}
      <div style={{ display:'flex', gap:8, marginBottom:14, alignItems:'center', background:'var(--parch-card)', border:'1px solid var(--parch-line)', borderRadius:10, padding:'8px 12px' }}>
        <input type="text" placeholder="Search products..." value={search} onChange={e => setSearch(e.target.value)}
          style={{ flex:1, background:'var(--parch-warm)', border:'1px solid var(--parch-line)', borderRadius:7, padding:'6px 10px', color:'var(--ink)', fontSize:12, outline:'none' }} />
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
          style={{ background:'var(--parch-warm)', border:'1px solid var(--parch-line)', borderRadius:7, padding:'6px 10px', color:'var(--ink-faded)', fontSize:11, outline:'none', cursor:'pointer' }}>
      ...
      {isLoading ? (
        ...
        <div style={{ background:'var(--parch-card)', borderRadius:12, border:'1px solid var(--parch-line)', overflow:'hidden' }}>
          <table style={{ width:'100%', borderCollapse:'collapse', tableLayout:'fixed' }}>
            <colgroup>
              <col style={{ width:28 }} /><col style={{ width:'34%' }} /><col style={{ width:'7%' }} />
              <col style={{ width:'12%' }} /><col style={{ width:'12%' }} /><col style={{ width:'12%' }} />
              <col style={{ width:'10%' }} /><col style={{ width:'13%' }} />
            </colgroup>
            <thead>
              <tr>
                <th style={th} /><th style={th}>Product</th><th style={th}>Qty</th>
                <th style={th}>Cost / unit</th><th style={th}>Total cost</th>
                <th style={th}>Est. profit</th><th style={th}>Status</th><th style={th} />
              </tr>
            </thead>
            <tbody>
              {filtered.map(group => (
                <GroupRow key={group.productName} group={group} expanded={!!expandedGroups[group.productName]}
                  onToggle={() => toggleGroup(group.productName)} onEdit={handleEdit} />
              ))}
            </tbody>
          </table>
        </div>
      ...
      {filtered.length > 0 && (
        <div style={{ marginTop:10, fontSize:10, color:'var(--ink-ghost)', textAlign:'right' }}>
      ...
      <POFormModal
        ...
        order={editingOrder}
        onSubmit={async (data) => {
          if (editingOrder) { await base44.entities.PurchaseOrder.update(editingOrder.id, data); queryClient.invalidateQueries({ queryKey: ['purchaseOrders'] }); }
          setFormOpen(false); setEditingOrder(null);
        }}
        products={products} creditCards={creditCards} giftCards={giftCards} sellers={sellers} isPending={false}
      />
    </div>
  );
}
...
if (n.includes(‘samsclub’) || n.includes(“sam’s”)) return ‘samsclub.com’;
...
/* unsold quantity for a given product name within an order */
...
const RETAILER_LIST = [‘Best Buy’, ‘Amazon’, ‘Walmart’, “Sam’s Club”, ‘Costco’, ‘Staples’, ‘Target’, ‘Apple’];
...
/* —————————————————————— */
/*  SECTION DIVIDER (matches Dashboard exactly)                        */
/* —————————————————————— */
...
/* —————————————————————— */
/*  SOURCE ORDER ROW (inside expanded card)                            */
/* —————————————————————— */
...
{/* Mark Received button – only show for non-received orders */}
{!isReceived && (
<button
onClick={handleMark}
disabled={marking}
...
/* accent color by dominant status */
const accentColor =
...
/* earliest delivery date across source orders */
...
```
  ...
  {/* footer: avg cost / total cost / est margin */}

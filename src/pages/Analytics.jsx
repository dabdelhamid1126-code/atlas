...
import { RefreshCw, Download, TrendingUp, DollarSign, ShoppingCart, Percent, CreditCard, Store, Package } from 'lucide-react';
...
export default function Analytics() {
  ...
  const load = async () => {
    ...
    setOrders(o); setRewards(r); setCreditCards(c);
    setLoading(false);
  };
  ...
  const periodData = useMemo(() => {
    ...
    return Object.values(map).sort((a, b) => a.period.localeCompare(b.period)).map(d => ({ ...d, profit: d.revenue - d.cost }));
  }, [filteredOrders, filteredRewards]);
  ...
  const storeData = useMemo(() => {
    ...
    return Object.values(map).map(s => ({ ...s, profit: s.sales - s.purchases, roi: s.purchases > 0 ? ((s.sales - s.purchases) / s.purchases * 100) : 0, spread: s.sales - s.purchases - s.cashback }));
  }, [filteredOrders, filteredRewards]);
  ...
  const platformData = useMemo(() => {
    ...
    return Object.values(map).map(p => ({ ...p, profit: p.sales - p.purchases, margin: p.sales > 0 ? ((p.sales - p.purchases) / p.sales * 100) : 0 }));
  }, [filteredOrders, filteredRewards]);
  ...
  const paymentData = useMemo(() => {
    ...
    return Object.values(map).map(p => ({ ...p, effectiveRate: p.spent > 0 ? (p.cashback / p.spent * 100) : 0, variance: p.spent > 0 ? ((p.cashback / p.spent * 100) - p.statedRate) : 0 }));
    ...
    <div className="min-h-screen bg-[#f4f5f9]">
      <div className="max-w-[1400px] mx-auto px-6 py-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3 mb-5">
          ...
          <KpiCard label="Profit" value={fmt$(kpis.profit)} icon={TrendingUp} color="bg-green-50 border-green-100" subtext={kpis.cost > 0 ? `${fmtPct((kpis.profit/kpis.cost)*100)} margin` : ''} />
          ...
          <KpiCard label="Inventory" value={kpis.inventory.toString()} icon={Package} color="bg-slate-50 border-slate-200" subtext="items ordered" />
        </div>

        {/* Tabs */}
        <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-2xl p-1 mb-5 shadow-sm w-fit flex-wrap">
          {tabs.map(t => (
            <button key={t.id} onClick={() => setActiveTab(t.id)}
              className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium transition ${activeTab === t.id ? 'bg-violet-100 text-violet-700 border border-violet-200' : 'text-slate-500 hover:bg-slate-50'}`}>
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                ...
                <p className="text-[10px] text-slate-400 mb-4">Revenue and profit over time</p>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
                    <Pie data={[{ name: 'COGS', value: kpis.cost }]} cx="50%" cy="50%" innerRadius={55} outerRadius={80} dataKey="value" paddingAngle={2}>
                      <Cell fill="#ef4444" />
                    </Pie>
                    <Tooltip contentStyle={chartTooltipStyle} formatter={v => fmt$(v)} />
                    <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 10 }} />
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={storeData.map(s => ({ store: s.store, roi: parseFloat(s.roi.toFixed(2)) }))}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm px-6 py-4 grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[

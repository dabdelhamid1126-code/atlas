...
export default function Transactions() {
  ...
  /* ------------------------------------------------------------------ */
  /*  STATS                                                               */
  /*  - totalCost uses final_cost (after gift cards) || total_cost       */
  /*  - totalSale reads sale_events[].items[].qty (entity schema)        */
  /*  - cashback filters to USD rewards only, matched to filtered orders */
  /* ------------------------------------------------------------------ */
  const stats = useMemo(() => {
    const totalItems = filteredOrders.reduce((s, o) => s + (o.items?.length || 0), 0);

    const totalCost = filteredOrders.reduce((s, o) =>
      s + parseFloat(o.final_cost || o.total_cost || 0), 0);
  ...
  const handleCSVDownload = () => {
    ...
    const rows = sortedOrders.map(o => {
      ...
      const cost   = parseFloat(o.final_cost || o.total_cost || 0);
      const profit = sale > 0 ? sale - cost : 0;
      const cb     = rewards.filter(r => r.currency === 'USD' && r.purchase_order_id === o.id).reduce((s, r) => s + (parseFloat(r.amount) || 0), 0);
      return [
        o.order_date || '', o.retailer || '', o.order_number || '',
        o.items?.length || 0, cost.toFixed(2), sale.toFixed(2),
        profit.toFixed(2), cb.toFixed(2),
  ...
  const profitAccent = stats.totalProfit > 0 ? 'var(--terrain)' : stats.totalProfit < 0 ? 'var(--crimson)' : 'var(--ink-ghost)';
  const profitBg     = stats.totalProfit > 0 ? 'var(--terrain-bg)' : stats.totalProfit < 0 ? 'var(--crimson-bg)' : 'var(--parch-warm)';
  const profitBdr    = stats.totalProfit > 0 ? 'var(--terrain-bdr)' : stats.totalProfit < 0 ? 'var(--crimson-bdr)' : 'var(--parch-line)';
  ...
  const STAT_CARDS = [
    ...
    { label:'Profit',     val: stats.totalSale > 0 ? fmt$(stats.totalProfit) : '--',
      Icon: TrendingUp, accent: profitAccent, bg: profitBg, bdr: profitBdr, valColor: stats.totalSale > 0 ? profitAccent : 'var(--ink-ghost)' },
  ...
  const INP = {
    ...
    border:'1px solid var(--parch-line)', background:'var(--parch-warm)',
    color:'var(--ink-dim)', outline:'none', cursor:'pointer',
  };
      ...
      <div style={{ background:'var(--parch-card)', border:'1px solid var(--parch-line)', borderRadius:10, padding:'12px 14px', marginBottom:14 }}>
        <div style={{ display:'flex', gap:8, flexWrap:'wrap', alignItems:'center' }}>
          <div style={{ flex:1, minWidth:180, position:'relative', display:'flex', alignItems:'center' }}>
            ...
            <input style={{ width:'100%', padding:'7px 10px 7px 32px', borderRadius:8, fontSize:12, border:'1px solid var(--parch-line)', background:'var(--parch-warm)', color:'var(--ink)', outline:'none' }}
              placeholder="Search products, stores, order #..." value={search} onChange={e => setSearch(e.target.value)}/>
          </div>
          ...
          <button style={{ display:'flex', alignItems:'center', gap:5, fontSize:11, color:'var(--ink-faded)', cursor:'pointer', padding:'6px 10px', borderRadius:7, border:'1px solid var(--parch-line)', background:'transparent', fontWeight:600, fontFamily:'var(--font-serif)' }}
              ...
              .map((p,i) => p==='...'
                ...
                : <button key={p} onClick={() => setCurrentPage(p)}
                    style={{ width:34, height:34, borderRadius:8, fontSize:12, cursor:'pointer', fontFamily:'var(--font-mono)',
                      fontWeight: p===safePage ? 700 : 500, border:'1px solid',

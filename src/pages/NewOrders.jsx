...
import {
  ...
  Minus, Check, ChevronDown as ChevronDownIcon,
} from 'lucide-react';
              ...
              <div style={{ maxHeight:240, overflowY:'auto' }}>
                {filteredCards.length === 0 ? (
                  ...
                  return (
                    ...
                    >
                      <div style={{ width:20, height:20, borderRadius:6, flexShrink:0, border:'1.5px solid ' + (isSelected ? 'var(--terrain)' : 'var(--parch-line)'), background: isSelected ? 'var(--terrain)' : 'var(--parch-warm)', display:'flex', alignItems:'center', justifyContent:'center', transition:'all 0.12s' }}>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'9px 12px', background:'var(--parch-warm)', borderTop:'1px solid var(--gold-bdr)' }}>
                <div>
                  <p style={{ fontSize:10, fontWeight:700, color:'var(--terrain)', fontFamily:'var(--font-mono)' }}>
                    {selectedCards.length} selected &middot; {fmt$(totalSelected)} applied
                  </p>
          ...
          {!open && selectedCards.length > 0 && (
            <div style={{ marginTop:8 }}>
              ...
              <div style={{ background:'var(--gold-bg)', border:'1px solid var(--gold-bdr)', borderRadius:10, padding:'10px 12px' }}>
                <p style={{ fontFamily:'var(--font-serif)', fontSize:8, fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', color:'var(--gold2)', marginBottom:8 }}>Applied Cards Summary</p>
        ...
        <div style={{ marginBottom:12 }}>
          ...
          <div style={{ display:'flex', gap:6, overflowX:'auto', paddingBottom:4, WebkitOverflowScrolling:'touch', scrollbarWidth:'none', msOverflowStyle:'none' }}>
            {QUICK_RETAILERS.map(r=>(
              <button key={r} type="button" onClick={()=>set('retailer',r)}
                style={{ display:'flex', alignItems:'center', gap:5, padding:'5px 10px', borderRadius:99, fontSize:11, fontWeight:600, cursor:'pointer', flexShrink:0,
                  border:'1px solid '+(form.retailer===r?'var(--gold-bdr)':'var(--parch-line)'),
                  background:form.retailer===r?'var(--gold-bg)':'var(--parch-card)',
                  color:form.retailer===r?'var(--gold2)':'var(--ink-faded)', transition:'all 0.12s' }}>

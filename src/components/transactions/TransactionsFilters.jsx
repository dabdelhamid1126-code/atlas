    ...
    <div style={{ background: 'var(--parch-card)', border: '1px solid var(--parch-line)', borderRadius: 12, padding: 16, marginBottom: 20 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'center' }}>
        ...
        <Select value={accountFilter} onValueChange={onAccountChange}>
          <SelectTrigger className="text-sm" style={{ ...inp, minWidth: 130 }}>
            <SelectValue placeholder="All Platforms">
              {accountFilter === 'all' ? 'All Platforms' : accountFilter}
            </SelectValue>
          </SelectTrigger>
          <SelectContent>
        ...
        >
          ...
          More filters
          {expanded && <span style={{ fontSize: 9, opacity: 0.7 }}>▲</span>}
        </button>
      </div>
      ...
      {expanded && (
        <div style={{
          marginTop: 14, paddingTop: 14, borderTop: '1px solid var(--parch-line)',
            ...
            <Select value={paymentMethodFilter} onValueChange={onPaymentMethodChange}>
              <SelectTrigger className="text-sm" style={inp}>
                <SelectValue placeholder="All Cards">
                  {paymentMethodFilter === 'all' ? 'All Cards' : (creditCards.find(c => c.id === paymentMethodFilter)?.card_name || paymentMethodFilter)}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>

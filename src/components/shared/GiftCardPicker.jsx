...
export default function GiftCardPicker({ giftCards=[], selectedIds=[], onChange, retailer='' }) {
  ...
  const select = (gc) => { onChange([...selectedIds, gc.id]); setQuery(''); inputRef.current?.focus(); };
    ...
    <div style={{ position:'relative', marginBottom:4 }}>
      ...
      <div
        ...
        style={{
          ...
          border:`1px solid ${focused ? 'var(--gold)' : 'var(--parch-line)'}`,
      ...
      >
        {selectedCards.map(gc => (
          <span key={gc.id} style={{
            ...
            borderRadius:20, color:'var(--rose)', fontSize:11, fontWeight:600,
          }}>
            ...
            <button type="button" onMouseDown={e => { e.stopPropagation(); remove(gc.id); }}
              style={{ color:'var(--rose)', lineHeight:1, marginLeft:2, background:'none', border:'none', cursor:'pointer', padding:0 }}>
        ...
        <input
          ...
          style={{ flex:1, minWidth:100, fontSize:12, color:'var(--ink)', background:'transparent', border:'none', outline:'none' }}
        />
      </div>
        ...
        <div style={{
          ...
          {filtered.length>0 ? filtered.slice(0,20).map(gc => (
            <button key={gc.id} type="button"
              ...
              style={{ width:'100%', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'9px 12px', background:'transparent', border:'none', cursor:'pointer', borderBottom:'1px solid var(--parch-line)', textAlign:'left' }}
              onMouseEnter={e => e.currentTarget.style.background='var(--gold-bg)'}
              onMouseLeave={e => e.currentTarget.style.background='transparent'}
            >
              <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                <Gift style={{ width:13, height:13, color:'var(--rose)', flexShrink:0 }} />
                <span style={{ color:'var(--ink)', fontSize:13, fontWeight:500 }}>{gc.brand}</span>
                {gc.code && <span style={{ color:'var(--ink-ghost)', fontSize:11 }}>••{String(gc.code).slice(-4)}</span>}
              </div>
              <span style={{ color:'var(--rose)', fontSize:13, fontWeight:600 }}>${parseFloat(gc.value).toFixed(2)}</span>
            </button>
          )) : (
            <div style={{ padding:12, textAlign:'center', color:'var(--ink-ghost)', fontSize:12 }}>No gift cards found</div>
          )}
        </div>
      )}
      ...
      {selectedCards.length>0 && (
        <div style={{
          ...
          background:'var(--rose-bg)', border:'1px solid var(--rose-bdr)', borderRadius:10,
        }}>
          <span style={{ color:'var(--rose)', fontSize:12, fontWeight:500 }}>
            {selectedCards.length} card{selectedCards.length!==1?'s':''} selected
          </span>
          <span style={{ color:'var(--rose)', fontSize:13, fontWeight:700 }}>
